def main(p_start_prompt, p_n_batches, p_batch_size, p_steps, p_batch_name, p_display_rate, p_init_image, p_init_scale, p_skip_steps, p_width_height):
    global batch_name, clip_guidance_scale, cutn_batches, init_image, init_scale, range_scale, sat_scale, skip_augs, skip_steps, steps, tv_scale, width_height

    global batch_size, display_rate, image_prompts, n_batches, text_prompts

    global RN101, RN50, RN50x16, RN50x4, RN50x64, ViTB16, ViTB32, ViTL14, ViTL14_336px, custom_path, diffusion_model, diffusion_sampling_mode, use_checkpoint, use_secondary_model, check_model_SHA
    ...
    batch_name, clip_guidance_scale, cutn_batches, init_image, init_scale, range_scale, sat_scale, skip_augs, skip_steps, steps, tv_scale, width_height = settings_basic_configs(batch_name , steps, init_image, init_scale, skip_steps, width_height)
    batch_size, display_rate, image_prompts, n_batches, text_prompts = setting_prompts_and_batchs(start_prompt, n_batches,  batch_size, display_rate)
    RN101, RN50, RN50x16, RN50x4, RN50x64, ViTB16, ViTB32, ViTL14, ViTL14_336px, custom_path, diffusion_model, diffusion_sampling_mode, use_checkpoint, use_secondary_model, check_model_SHA = setting_model()
...
def run_draw():
    ...
    def gitclone(url, targetdir=None):
        if targetdir:
            print(f'git clone from {url} to path: {targetdir}')
    ...
    def pipi(modulestr):
        print(f'pip installing {modulestr}')
        res = subprocess.run(['pip', 'install', modulestr], stdout=subprocess.PIPE).stdout.decode('utf-8')
        print(res)

    def pipie(modulestr):
        print(f'git install -e {modulestr}')
    ...
    class MakeCutoutsDango(nn.Module):
        ...
        def forward(self, input):
            ...
            if self.Overview > 0:
                ...
                if cutout_debug:
                    if is_colab:
                        TF.to_pil_image(cutouts[0].clamp(0, 1).squeeze(0)).save("/content/cutout_overview0.jpg",
                                                                                quality=99)
                    else:
                    ...
                    if turbo_mode:
                        if frame_num == turbo_preroll:  # start tracking oldframe
                            ...
                            if frame_num % int(turbo_steps) != 0:
                                ...
                                blendedImage = cv2.addWeighted(newWarpedImg, blend_factor, oldWarpedImg,
                                                               1 - blend_factor,
                                                               0.0)
                                cv2.imwrite(f'{batchFolder}/{filename}', blendedImage)
                                next_step_pil.save(
                                    f'{img_filepath}')  # save it also as prev_frame to feed next iteration
                                if vr_mode:
    ...
    def generate_eye_views(trans_scale, batchFolder, filename, frame_num, midas_model, midas_transform):
        for i in range(2):
            ...
            rot_mat = p3dT.euler_angles_to_matrix(torch.tensor(rotate_xyz, device=device), "XYZ").unsqueeze(0)
            transformed_image = dxf.transform_image_3d(f'{batchFolder}/{filename}', midas_model, midas_transform,
                                                       DEVICE,
                                                       rot_mat, translate_xyz, args.near_plane, args.far_plane,
                                                       args.fov, padding_mode=args.padding_mode,
                                                       sampling_mode=args.sampling_mode, midas_weight=args.midas_weight,
    ...
    clamp_grad, clamp_max, clip_denoised, eta, fuzzy_prompt, perlin_init, perlin_mode, rand_mag, randomize_class, set_seed = setting_advanced()
...
def settings_basic_configs(batch_name = 'DefaultDisco', steps = 250, init_image = None, init_scale = 1000, skip_steps = 10, width_height = [1280, 768]):
    # @markdown ####**Basic Settings:**
    # batch_name = 'DefaultDisco'  # @param{type: 'string'}
...
def setting_prompts_and_batchs(start_prompt = ["A beautiful painting of Japanese city and tradition temple by Artgerm, Trending on artstation."],
    ...
    # display_rate = 1000  # @param{type: 'number'}
...
def cli_main():
    ...
    parser = argparse.ArgumentParser(
        description=(
            "A simple cli tool to run disco diffusion"
        )
    )

    parser.add_argument(
        "-n", "--n-batches", type=int, default=1,
        help=("numbers of imgs to render")
    )

    parser.add_argument(
        "-t", "--start-prompt", nargs='+', default=["Japanese city", "human:-2"],
        help=("text prompt to render")
    )

    parser.add_argument(
        "-w", "--width_height", nargs='+', type=int, default=[1280, 768],
        help=(  "width and height"
                "e.g. -w 1280 768"
                "takes two argument only and ignore the rest"
        )
    )
    ...
    parser.add_argument(
        "-s", "--steps", type=int, default=250,
        help=("steps of img")
    )

    parser.add_argument(
        "-d", "--display-rate", type=int, default=25,
        help=("display rate of img while rendering, location: ./progess.png")
    )

    parser.add_argument(
        "-o", "--output", type=str,  default="DefaultDisco",
        help=("output name of img")
    )

    parser.add_argument(
        "-I", "--init-image", type=str, default=None,
        help=("init noise from img")
    )

    parser.add_argument(
        "-S", "--skip-steps", type=int, default=10,
        help=("skip step while init noise from img")
    )

    parser.add_argument(
        "--init-scale", type=int, default=1000,
        help=("the init scale for init image")
    )

    args = parser.parse_args()

    p_start_prompt = args.start_prompt
    p_n_batches = args.n_batches
    p_batch_size = 1
    p_batch_name = args.output
    p_steps = args.steps
    p_display_rate = args.display_rate
    p_init_image = args.init_image
    p_skip_steps = args.skip_steps
    p_init_scale = args.init_scale
    p_width_height = args.width_height

    if len(p_width_height) == 1:
        p_width_height.append(p_width_height[-1])
    elif len(p_width_height) > 2:
        p_width_height = p_width_height[:2]

    print(f"start with args, [{p_batch_name},{p_start_prompt},{p_n_batches},{p_batch_size},{p_steps}, {p_display_rate}, {p_width_height}]")
    if p_init_image:
        print(f"using init image: {p_init_image}, args: [{p_skip_steps},{p_init_scale}]")
    main(p_start_prompt, p_n_batches, p_batch_size, p_steps, p_batch_name, p_display_rate, p_init_image, p_init_scale, p_skip_steps, p_width_height)
    print(f"Diffusion Done, folder:{p_batch_name}, count:{p_n_batches}")
